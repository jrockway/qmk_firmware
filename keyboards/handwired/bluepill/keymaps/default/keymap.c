#include QMK_KEYBOARD_H

#define _NORMAL 0

// clang-format off
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	/* [_NORMAL] = { */
	/* 	{KC_A, KC_B, KC_C, KC_D, KC_E, KC_F, KC_G, KC_H, KC_I, KC_J, KC_K, KC_L, KC_M, KC_N, KC_O, KC_P, KC_Q, KC_R, KC_S, KC_T, KC_U}, */
	/* 	{KC_A, KC_B, KC_C, KC_D, KC_E, KC_F, KC_G, KC_H, KC_I, KC_J, KC_K, KC_L, KC_M, KC_N, KC_O, KC_P, KC_Q, KC_R, KC_S, KC_T, KC_U}, */
	/* 	{KC_A, KC_B, KC_C, KC_D, KC_E, KC_F, KC_G, KC_H, KC_I, KC_J, KC_K, KC_L, KC_M, KC_N, KC_O, KC_P, KC_Q, KC_R, KC_S, KC_T, KC_U}, */
	/* 	{KC_A, KC_B, KC_C, KC_D, KC_E, KC_F, KC_G, KC_H, KC_I, KC_J, KC_K, KC_L, KC_M, KC_N, KC_O, KC_P, KC_Q, KC_R, KC_S, KC_T, KC_U}, */
	/* 	{KC_A, KC_B, KC_C, KC_D, KC_E, KC_F, KC_G, KC_H, KC_I, KC_J, KC_K, KC_L, KC_M, KC_N, KC_O, KC_P, KC_Q, KC_R, KC_S, KC_T, KC_U}, */
	/* 	{KC_A, KC_B, KC_C, KC_D, KC_E, KC_F, KC_G, KC_H, KC_I, KC_J, KC_K, KC_L, KC_M, KC_N, KC_O, KC_P, KC_Q, KC_R, KC_S, KC_T, KC_U}, */
	/* } */
	[_NORMAL] = LAYOUT_ergorocks(
		KC_ESC, KC_F1, KC_F2, KC_F3, KC_F4, KC_F5, KC_F6,
		KC_GRAVE, KC_1, KC_2, KC_3, KC_4, KC_5, KC_MINUS,
		KC_TAB, KC_Q, KC_W, KC_E, KC_R, KC_T, KC_KP_MINUS,
		KC_LCTRL, KC_A, KC_S, KC_D, KC_F, KC_G, KC_KP_PLUS,
		KC_LSHIFT, KC_Z, KC_X, KC_C, KC_V, KC_B, KC_RCTRL,
		KC_LGUI, KC_LCTRL, KC_LALT, KC_X, KC_LALT, KC_SPC,

		KC_PSCR, KC_SLCK, KC_INS, KC_DEL,
		KC_NLCK, KC_KP_SLASH, KC_KP_ASTERISK, KC_KP_MINUS,
		KC_KP_7, KC_KP_8, KC_KP_9,
		KC_KP_4, KC_KP_5, KC_KP_6, KC_KP_PLUS,
		KC_KP_1, KC_KP_2, KC_KP_3,
		KC_KP_0, KC_KP_DOT, KC_KP_ENTER,

		KC_UP, KC_LEFT, KC_DOWN, KC_RIGHT,

		KC_F7, KC_F8, KC_F9, KC_F10, KC_F11, KC_F12, KC_PAUSE,
		KC_EQUAL, KC_6, KC_7, KC_8, KC_9, KC_0, KC_BSLS,
		KC_PGUP, KC_Y, KC_U, KC_I, KC_O, KC_P, KC_RBRC,
		KC_PGDN, KC_H, KC_J, KC_K, KC_L, KC_SCLN, KC_QUOT,
		KC_BSPC, KC_N, KC_M, KC_COMMA, KC_DOT, KC_SLASH, KC_RSHIFT,
		KC_ENTER, KC_SPACE, KC_X, KC_RALT, KC_APP, KC_RGUI
	),
};
// clang-format on
